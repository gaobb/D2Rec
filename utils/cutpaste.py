...
class CutPaste(object):
    ...
    def cutpaste(self, image, mask, area_ratio = (0.02, 0.15), aspect_ratio = ((0.3, 1) , (1, 3.3))):
        ...
        if patch_w > image.size[0]:
            patch_w = image.size[0]-1
          
        if patch_h > image.size[1]:
            patch_h = image.size[1]-1
...
class PerlinPaste(object):
    def __init__(self):
        ...
        self.anomaly_paths = sorted(glob.glob("/fuxi_team2/persons/danylgao/weicun_ceph/datasets/dtd/images"+"/*/*.jpg"))
